...
outputs_16bit = ['down_proj','embed_tokens','attn_residue','mlp_residue','eltmul','sfmx','o_proj','lm_head'] + outputs_8bit
outputs_8bit = []
			...
			if "lm_head" in name:
				...
				for pattern in patterns_3:
					if pattern in name:
						...
						continue
				if not flag:
					print(f"Quantizing the dp weight of the Layer:{name}, bits: 3, group_size: 32", module.weight.shape)
					module.weight = torch.nn.Parameter(qdq(module.weight,3, 32, True))
			# else:
			# 	print(f"Quantizing the dp weight of the Layer:{name}, bits: 3, group_size: 64")
...
def calculate_scale_offsets(min_max_dict, n_bits = 8 ,offset_enable = False):
	...
	for layer in min_max_dict:
		...
		abs_max = max(map(abs, min_max_dict[layer])).item()
...
#   generate_ids = model.generate(input_ids = inputs.input_ids.to('cuda'),do_sample=True,temperature=0.7,top_k=15,top_p=0.9,repetition_penalty=1.2)
  generate_ids = model.generate(input_ids = inputs.input_ids.to('cuda'),max_new_tokens=1000)
...
exit()
# dump_json("/auto/worka/dgundimeda/llama2_onnx/Qwen/output_stats.json",output_stats)
output_scale_offsets = calculate_scale_offsets(output_stats)
# with open("/auto/worka/dgundimeda/llama2_onnx/Qwen/output_scale_offsets_QAT2.json", 'w') as json_file:
...
prompt = "Write a short description about LLM(Large Language Model)."
print("Prompt: ", prompt)
result = prompt_inference(model, tokenizer, prompt)
