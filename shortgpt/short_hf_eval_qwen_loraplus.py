...
parser.add_argument("--prune_layers", type=int, default=6, help="Number of layers to prune")
parser.add_argument("--seed", type=int, default=10, help="Seed for sampling the calibration data.")
parser.add_argument("--pruned_path", default="./pruned_model_loraplus_tmp1", type=str, help="Path to save the pruned model")
parser.add_argument("--lora_trained_path", default="./peftmodel_qwen_loraplus_tmp1", type=str, help="Path to save the trained peft model")
parser.add_argument("--merged_path", default="./merged_model_loraplus_tmp1", type=str, help="Path to save the merged model")
...
data = load_dataset('allenai/c4', data_files="en/c4-train.00001-of-01024.json.gz", split = f"train[:{samples}]")
# data = load_dataset('allenai/c4', data_files="multilingual/c4-zh-validation*.json.gz", split = f"train[:{samples}]")

data = data['text']

for index, text in enumerate(data):
    data[index] = trim_string(text, 256)
...
# train_dataset = get_preprocessed_ceval(short_model.tokenizer, "train", train_samples, False, 128)
...
gen = model.generate(
    tokenizer(["Value of 1+2+4 ="], return_tensors='pt').input_ids.to("cuda"),
    max_new_tokens=40
)
new_textgen = tokenizer.batch_decode(gen, skip_special_tokens=True)
