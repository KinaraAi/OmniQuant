...
class ShortLlama():
	...
	@torch.inference_mode()
	def perplexity(self, dataloader, device="cuda"):
		"""
		Calculates perplexity on a given dataset using a dataloader.

		Args:
			dataloader (DataLoader): PyTorch DataLoader object for the dataset.
			tokenizer: Tokenizer object for converting text to tokens.
			device (str, optional): Device to use for computation (e.g., "cuda"). Defaults to "cuda".

		Returns:
			float: Perplexity score of the model on the dataset.
		"""
		seqlen = 2048
		total_loss = 0
		num_tokens = 0
		...
		results = {}
		dataset  = "wiki2"
		nsamples = min(200,testenc.numel() // seqlen)
		with torch.no_grad():
			# for batch in tqdm(dataloader):
			for i in tqdm(range(nsamples)):
				batch = testenc[:, (i * seqlen) : ((i + 1) * seqlen)].to('cuda')
				# text = batch["text"]  # Assuming the dataset has a "text" key
				# tokens = self.llama.tokenizer.encode(text,bos=True, eos=False).to(device)
				# prompts = batch['text']
				# tokens = [self.llama.tokenizer.encode(x, bos=True, eos=False) for x in prompts]
				# tokens = torch.tensor(tokens, dtype=torch.long, device="cuda")
				outputs = self.llama.model(batch, 0)
		...
		results[dataset] = ppl.item()
				...
				# loss_fct = torch.nn.CrossEntropyLoss(ignore_index=self.llama.tokenizer.pad_id)
				# loss = loss_fct(logits.reshape(-1, logits.size(-1)), batch.reshape(-1))
				# total_loss += loss.item()

				# # Count non-padding tokens
				# mask = batch != self.llama.tokenizer.pad_id
				# num_tokens += mask.sum().item()

		# Calculate and return perplexity (exponentiated average negative log-likelihood)
