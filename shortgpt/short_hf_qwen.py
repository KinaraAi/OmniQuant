...
class ShortHFModel():
    ...
    def __init__(self, model_name: str, layers_path: str, quantization_config = None, n_prune_layers: Optional[int] = None):
        """
        ...
        """
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, pad_token='<|endoftext|>',trust_remote_code=True)#, padding='max_length', max_length = 2048)
    ...
    def remove_layers(
        ...
        if angular:
            ...
            # breakpoint()
            x = -self.n_prune_layers+1
            if self.n_prune_layers == 1:
                x = -1
            start_layer = np.argsort(np.array(self.importances[:x]))[0]
        ...
        layers_to_remove = [21,22,23,25,26,27]
    ...
    def compute_bi(self, hiddens: List[torch.Tensor], angular: bool):
        ...
        for i in range(min(32,len(hiddens) - n)):
            in_hidden = hiddens[i]
    ...
    def eval_importance(
        ...
        with torch.no_grad():
            for start in range(0, max_prompt_len, stride):
                ...
                del inputs, outputs, attn, seq_ids
